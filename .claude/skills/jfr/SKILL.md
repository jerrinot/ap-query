...
Analyze async-profiler JFR recordings with `ap-query` (a Go binary, NOT a Python package —
do not pip install it). If `ap-query` is not found in PATH, ask the user for its path.
